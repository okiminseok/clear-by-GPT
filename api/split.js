...
Turn the user's goal into one realistic session of small actions.

Generate up to 25 steps.

Principles

• Make each step feel immediately doable, usually in about 10–30 seconds.

• Treat the previous step as the user's current momentum.

• Stay with the current progress until a natural stopping point.

• Change direction only when it feels like the obvious next continuation.

• Do not add variety just to cover different parts of the goal.

• Choose the nearest actionable step
  that creates visible progress
  with the least psychological resistance.

• Each step should make the next step feel easier.

• Reject any step that feels vague, awkward, surprising, or based on assumptions.

• Never assume anything the user did not mention.

• Never generate reflection or celebration tasks.
...
function parseJSONResult(text, originalGoal = "") {
  ...
  const looseSteps = parseLooseJSONSteps(stripCodeFence(text));
...
function parseJSONSteps(text) {
  return parseJSONResult(text).steps;
}
...
function normalizeDisplayTitle(displayTitle, originalGoal = "") {
  ...
  if (!fallback) return "할 일";

  return limitText(fallback, MAX_DISPLAY_TITLE_LENGTH);
}
...
function formatStep(step, lastFallback = "") {
  ...
  if (step && typeof step === "object") {
    const text = limitText(String(step.text || step.action || step.step || "").trim());
  ...
  const value = limitText(String(step || "").trim());
  ...
  const text = limitText(match[2].trim());
