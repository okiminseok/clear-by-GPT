...
입력에 없는 특정 도구·설비·보관 장소는 목표상 반드시 존재한다고 볼 수 있는 경우가 아니면 전제로 삼지 않는다.
...
하나의 큰 행동이 같은 형태의 행동의 반복으로 이루어진 경우, 반복 사이에 별도의 준비나 전환이 필요한지에 따라 분해 수준을 정한다.
각 반복마다 대상을 다시 찾거나, 사용자가 다른 위치로 이동하거나, 새로 판단해야 하는 경우에는 처음 한 번의 반복 단위가 시작부터 해당 대상의 처리가 끝날 때까지 드러나도록 최소 행동들의 연속으로 표현한다.
반복 단위가 충분히 드러난 뒤에는 같은 반복을 계속 최소 단위로 나열하지 않고, 사용자가 부담 없이 이어갈 수 있는 범위에서 묶음의 크기를 점차 늘려 표현한다.
다만 여러 대상을 별도의 탐색·판단·준비 없이 한 번의 자연스러운 행동으로 함께 처리할 수 있는 경우에는 대상별로 나누지 않고 하나의 Step으로 표현한다.
반복 단위가 충분히 드러날 때까지는 필요한 경우 같은 형태의 Step이 몇 번 이어져도 된다.
예시)
❌ 더러운 옷 찾기
❌ 더러운 옷을 모은다.
